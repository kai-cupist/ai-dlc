...
# Private route table (shared by app and db subnets)
...
resource "aws_vpc_security_group_egress_rule" "alb_to_ecs" {
  security_group_id            = aws_security_group.alb.id
  description                  = "Allow outbound to ECS on container port"
...
resource "aws_vpc_security_group_egress_rule" "ecs_to_https" {
  security_group_id = aws_security_group.ecs.id
  description       = "Allow outbound HTTPS (external APIs, S3 via endpoint)"
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 443
  to_port           = 443
  ip_protocol       = "tcp"
...
resource "aws_vpc_security_group_ingress_rule" "rds_from_ecs" {
  ...
  description                  = "Allow inbound from ECS on PostgreSQL port"
  referenced_security_group_id = aws_security_group.ecs.id
  from_port                    = 5432
  to_port                      = 5432
  ip_protocol                  = "tcp"
